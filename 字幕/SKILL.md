---
name: videocut:烧录字幕
description: 字幕生成与烧录。火山引擎转录→词典纠错→审核→烧录。触发词：烧录字幕、加字幕、生成字幕、字幕
---
...
2. **语音剪辑产出的 `_cut.mp4`**：在当前 output 目录下查找 `语音剪辑/3_审核/*_cut.mp4`
3. **原始视频**：用户最初传入语音剪辑的原始视频
...
# 优先找剪辑后视频
CUT_VIDEO=$(find "$OUTPUT_DIR/语音剪辑/3_审核" -name "*_cut.mp4" -type f 2>/dev/null | head -1)
...
**关键**：字幕应该基于**剪辑后的视频**生成，而不是原始视频。因为语音剪辑会删除口误和静音段，原始视频的时间戳与剪辑后不匹配。
...
bash ../语音剪辑/scripts/volcengine_transcribe.sh "https://o.uguu.se/xxxxx.mp3"
```
...
- **原稿没有 = 必须删除**：字幕基于 _cut.mp4（剪辑后视频），原稿里没有的内容说明在语音剪辑阶段已经被剪掉了，转录出现是火山引擎的幻觉或错误识别，必须删除
